...
export const generateXRPCOutput = (md: MarkdownRenderer, parsed: ReturnType<typeof parseXRPCContent>) => {
...
`.trim(),
	...
	const tabs = snippets.map((snippet, idx) => (`
		<input type="radio" id="tab-${idx}" name="xrpc-tabs" ${idx === 0 ? "checked" : ""}>
		<label for="tab-${idx}" data-title="${md.utils.escapeHtml(snippet.title)}">${snippet.title}</label>
	`)).join("");

	const blocks = snippets.map((snippet) => (
		md.render(`\`\`\`${snippet.language} [${snippet.title}]\n${snippet.code}\n\`\`\``)
	)).join("");
...
export const ExtXRPC = (md: MarkdownRenderer) => {
	...
	md.renderer.rules.fence = (tokens, idx, options, env, self) => {
		...
		if (token.info.trim() === "xrpc")
			return generateXRPCOutput(md, parseXRPCContent(token.content));
